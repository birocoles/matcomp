...
def _FT_recursive(data, conjugate=False):
    ...
    if N <= 64:
        ...
        return np.hstack([y_even + omega * y_odd, y_even - omega * y_odd])
...
def QR_House(A, check_input=True):
    ...
    for j in range(N):
        ...
        A[j:, j:] = House_matvec(
            A=A[j:, j:], v=v, beta=beta, order="PA", check_input=False
        )
        # if j < M:
        #     A[j+1:,j] = v[1:M-j+2]
...
def Q_from_QR_House(A, check_input=True):
    ...
    for j in range(N - 2, -1, -1):
        ...
        Q[j:, j:] = House_matvec(
            A=Q[j:, j:], v=v, beta=beta, order="PA", check_input=False
        )
...
def QR_Givens(A, check_input=True):
    ...
    for j in range(N):
        for i in range(M - 1, j, -1):
            ...
            Givens_matvec(
                A[:, j:], c, s, i - 1, i, order="GTA", check_input=False
            )
...
def upper_Hessen_House(A, check_input=True):
    ...
    for k in range(N - 1):
        ...
        A[k + 1 :, k:] = House_matvec(
            A=A[k + 1 :, k:], v=v, beta=beta, order="PA", check_input=False
        )
        A[:, k + 1 :] = House_matvec(
            A=A[:, k + 1 :], v=v, beta=beta, order="AP", check_input=False
        )
...
def U0_from_upper_Hessen_House(A, check_input=True):
    ...
    for k in range(N - 3, -1, -1):
        ...
        U0[k + 1 :, k:] = House_matvec(
            A=U0[k + 1 :, k:], v=v, beta=beta, order="PA", check_input=False
        )
...
def Francis_QR_step(H, check_input=True, return_v=True):
    ...
    for k in range(N - 3):
        ...
        H[k + 1 : k + 4, k:] = House_matvec(
            A=H[k + 1 : k + 4, k:], v=v, beta=beta, order="PA"
        )
        H[: k + 5, k + 1 : k + 4] = House_matvec(
            H[: k + 5, k + 1 : k + 4], v=v, beta=beta, order="AP"
        )
    ...
    H[N - 2 :, N - 3 :] = House_matvec(
        A=H[N - 2 :, N - 3 :], v=v, beta=beta, order="PA"
    )
...
def House_tridiag(A, check_input=True):
    ...
    for k in range(N - 2):
        ...
        # zero kth column
        A[k + 1 :, k:] = House_matvec(
            A=A[k + 1 :, k:], v=v, beta=beta, order="PA", check_input=True
        )
        # zero kth row
        A[k:, k + 1 :] = House_matvec(
            A=A[k:, k + 1 :], v=v, beta=beta, order="AP", check_input=True
        )
        # store the Householder vectors in the lower triangle of A
        A[k + 2 :, k] = v[1:]
...
def Q_from_House_tridiag(A, check_input=True):
    ...
    for k in range(N - 3, -1, -1):
        ...
        v = np.hstack([1, A[k + 2 :, k]])
        beta = 2 / (1 + np.dot(A[k + 2 :, k], A[k + 2 :, k]))
        Q[k + 1 :, k:] = House_matvec(
            A=Q[k + 1 :, k:], v=v, beta=beta, order="PA", check_input=False
        )
...
def imp_symm_QR_step_shift(T, check_input=True):
    ...
    if check_input is True:
        ...
        assert np.all(
            np.diag(v=T, k=1) == np.diag(v=T, k=-1)
        ), "super and \
subdiagonals must be equal to each other"
    ...
    d = 0.5 * (T[-2, -2] - T[-1, -1])
    mu = T[-1, -1] - (T[-1, -2] ** 2) / (
        d + np.sign(d) * np.sqrt(d ** 2 + T[-1, -2] ** 2)
    )
    ...
    while q != N:
        ...
        if q < N:
            ...
            if zeros_diag_B22.size > 0:
                ...
                B[p : N - q, p : N - q] = Golub_Kahan_SVD_step(
                    alpha=alpha_B22, beta=beta_B22
                )
    return B
...
def sd_lsearch(A, dobs, p0, tol, itmax):
    ...
    assert A.shape[0] == A.shape[1], "A must be square"
    assert (
        dobs.size == A.shape[0] == p0.size
    ), "A order, dobs size and p0 size must be the same"
    assert np.isscalar(tol) & (tol > 0.0), "tol must be a positive scalar"
    assert isinstance(itmax, int) & (
        itmax > 0
